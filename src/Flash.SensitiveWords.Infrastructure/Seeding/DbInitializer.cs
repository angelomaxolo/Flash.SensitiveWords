...
namespace Flash.SensitiveWords.Infrastructure.Seeding;
...
public sealed class DbInitializer : IDbInitializer
{
    ...
    public DbInitializer(SensitiveWordsDbContext dbContext)
    {
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (await _dbContext.SensitiveWords.AnyAsync(cancellationToken))
        {
            return;
        }

        var initialWords = new[]
        {
        ...
        _dbContext.SensitiveWords.AddRange(initialWords.Select(word => new SensitiveWord(word)));
